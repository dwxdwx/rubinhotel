$(function() {
	$("#productTable").bootstrapTable({
		url: 'orderservlet?method=getAll', // 初始化加载数据来源
		method: 'get',
		...
		queryParams: function(params) {
			...
			params.nameKey = $("#search_uname").val();
	...
	$("#btn_delete").click(function() {
		...
		bootbox.confirm('确认删除?', function(confirmR) {
			if(confirmR) {
				...
				// ajax请求删除
				$.get(
					'orderservlet?method=delMore&delIds=' + idStr,
					function(result) {
						if(result == "true") {
							bootbox.alert('删除成功！');
							$("#productTable").bootstrapTable('refresh');
...
window.operateEvents = {
	...
	'click .edit': function(e, value, row, index) { // 参数row是本行的数据
		...
		// bootstrap -- 模态框
		// 模态框显示数据
		$("#editWindow #edit_oid").val(row.oid);
		$("#editWindow #edit_uname").val(row.uname);
		$("#editWindow #edit_omoney").val(row.omoney);
		$("#editWindow #edit_ostate").val(row.ostate);
		...
		$("#editSubmitBtn").one('click', function() {
			// ajax 提交更新
			$.post(
				'orderservlet?method=updateOrder',
				$("#editForm").serialize(),
				function(result) {
					if(result == "true") {
						bootbox.alert('更新成功!');
	...
	'click .remove': function(e, value, row, index) {
		...
		bootbox.confirm('确认删除?', function(confirmR) {
			if(confirmR) {
				// ajax 执行删除操作
				$.get(
					'orderservlet?method=delOne&delId=' + row.oid,
					function(result) {
						if(result == "true") {
							bootbox.alert('删除成功！');
							$("#productTable").bootstrapTable('refresh');
