$(function() {
	$("#productTable").bootstrapTable({
		url: 'roomservlet?method=getAll', // 初始化加载数据来源
		method: 'get',
		...
		queryParams: function(params) {
			...
			params.nameKey = $("#search_rtype").val();
...
//			// ajax 提交更新
//			$.post(
//				'../ProductServlet?method=addProduct',
//				$("#addForm").serialize(),
//				function(result) {
//					if(result == "true") {
//						bootbox.alert('添加成功!');
...
//				// ajax请求删除
//				$.get(
//					'../ProductServlet?method=delMore&delIds=' + idStr,
//					function(result) {
//						if(result == "true") {
//							bootbox.alert('删除成功！');
//							$("#productTable").bootstrapTable('refresh');
...
window.operateEvents = {
	...
	'click .edit': function(e, value, row, index) { // 参数row是本行的数据
		...
		// bootstrap -- 模态框
		// 模态框显示数据
		$("#editWindow #edit_rid").val(row.rid);
		$("#editWindow #edit_rstate").val(row.rstate);
		// 模态框显示
		$("#editWindow").modal("show");
		$("#editSubmitBtn").one('click', function() {
			// ajax 提交更新
			$.post(
				'roomservlet?method=updateRoom',
				$("#editForm").serialize(),
				function(result) {
					if(result == "true") {
						bootbox.alert('更新成功!');
...
//				// ajax 执行删除操作
//				$.get(
//					'../ProductServlet?method=delOne&delId=' + row.id,
//					function(result) {
//						if(result == "true") {
//							bootbox.alert('删除成功！');
//							$("#productTable").bootstrapTable('refresh');
